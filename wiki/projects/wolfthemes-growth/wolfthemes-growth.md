...
- **Newsletter blocks support both Mailchimp and Brevo (2026-06-27):** [[wolf-blocks]] now ships **two sibling blocks** — `wolf-blocks/mailchimp-form` and `wolf-blocks/brevo-form` — sharing one implementation behind a `Subscription_Provider` interface (Mailchimp/Brevo providers own each API call; one REST route dispatches by `provider` param). One settings page **"Newsletter Settings"** holds both API keys (sanitize guard prevents saving one key from wiping the other). Shared editor component, render (`Subscription_Block`), frontend binder and stylesheet (`src/shared/*`) → both blocks look/behave identically. For the launch, use the **Brevo Form** block (numeric Brevo list ID) and enable **double opt-in** on the target list so footer sign-ups are consent-clean. Brevo: `POST /v3/contacts`, `api-key` header, integer `listIds`, `updateEnabled`.
